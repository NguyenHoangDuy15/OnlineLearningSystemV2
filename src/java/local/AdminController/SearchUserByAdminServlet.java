 ...
 * @author DELL
 */
@WebServlet(name="SearchUserByAdminServlet", urlPatterns={"/SearchUserByAdminServlet"})
public class SearchUserByAdminServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            name = name.trim();
            int NoPage = getNoPage(udao.findUserBySearch(name));
            ...
            List<User> listUser = udao.get5UserBySearch(index, name);
            session.setAttribute("Nopage", NoPage);
            session.setAttribute("currentindex", index);
            session.setAttribute("listUser", listUser);
            request.getRequestDispatcher("jsp/ListOfUserByAdmin.jsp").forward(request, response);
        } catch (Exception e) {
